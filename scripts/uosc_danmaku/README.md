...
开启此选项后，会为可能支持的 url 视频文件实现弹幕关联记忆和继承，配合播放列表食用效果最佳。目前兼容在使用[embyToLocalPlayer](https://github.com/kjtsune/embyToLocalPlayer)、[tsukimi](https://github.com/tsukinaha/tsukimi)等场景时进行弹幕关联记忆和继承。
...
本整合包默认使用 `overlay`：弹幕以独立时钟最高 60Hz 重绘，不占用第二字幕轨，也不会修改源视频帧率。`auto` 仍可作为兼容模式，优先把弹幕生成为临时 ASS 字幕轨；检测到已有第二字幕、用户中途选择第二字幕或 ASS 轨加载异常时会回退 `overlay`，并恢复原字幕状态。

`overlay_fps` 可限制刷新率，范围为 20 到 60，实际不会超过显示器刷新率。

`overlay_display_sync=yes` 会在低于约 59fps 的滚动 overlay 弹幕出现时，临时使用 `display-vdrop` 让 mpv 随显示器刷新重复绘制当前视频帧，使最高 60Hz 的弹幕坐标更新真正可见。它不生成插值视频帧、不改变音频速度，也不会把 `fps` 滤镜放回 HDR 链路。只接管默认 `video-sync=audio`；已有 `display-*` 模式、倍速偏离 1×、低于 58Hz 的显示器、接近 60fps 的片源和 SDR 已存在 `@danmaku:fps` 时不会重复介入。关闭弹幕、恢复原生 ASS 或卸载文件后会按 `copy-equal` 语义恢复，用户或其他配置中途写入的新值不会被覆盖。高刷新率下视频输出会随显示器更频繁重绘，如设备性能不足可设置为 `no`。
...
当该值大于0时，脚本会在解析弹幕时丢弃部分弹幕，确保任意时刻屏幕中显示的弹幕不超过设定值。建议保留默认值 120，避免极端高密度弹幕拖慢渲染；设为 0 可取消限制。
...
旧版兼容选项。启用后，仅在显式使用 ASS 轨模式时允许添加 `@danmaku:fps` 视频滤镜；默认 `overlay` 模式始终保持视频滤镜链不变。整合包已设为 `vf_fps=no`，由 overlay 最高 60Hz 独立重绘弹幕，因此 23.976/24/30fps 视频不会被复制成 60fps。

当视频传递特性尚未就绪时，脚本会暂缓添加 fps 滤镜，避免 HDR 起播阶段先插入再移除而引起瞬时切换。
...
自定义 b 站和爱腾优的弹幕获取兜底服务器地址，主要用于获取非动画弹幕。只有在弹弹play兼容接口、站点原生解析和扩展弹幕接口均未返回有效结果时，才会使用此处设置的服务器解析具体视频链接。当前可用： https://dmku.hls.one
