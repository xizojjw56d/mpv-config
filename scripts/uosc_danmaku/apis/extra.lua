...
function get_details(class, id, site, title, year, number, episodenum)
    ...
    if uosc_available and not episodenum then
        update_menu_uosc(menu_type, menu_title, message, footnote,
            nil, nil, "spinner", nil, nil, episode_menu_options)
    else
        show_message(message, 3)
    end
    ...
    if not episode_rows then
        ...
        if episodes then
            ...
            if not number or cat == 0 then
                ...
                if uosc_available and not episodenum then
                    update_menu_uosc(menu_type, menu_title, message, footnote,
                        nil, nil, nil, nil, nil, episode_menu_options)
                else
                    show_message(message, 3)
                end
                msg.verbose("无结果")
...
local function build_extra_search_url(query, class)
...
function make_extra_search_request_args(query, class, connect_timeout, request_timeout)
    local args = { "curl", "-s" }
    ...
    if request_timeout then
        ...
        table.insert(args, tostring(request_timeout))
    end
    if connect_timeout or request_timeout then
        table.insert(args, "--fail")
    end
    table.insert(args, build_extra_search_url(query, class))
    return args
end

local function parse_extra_search_items(stdout)
...
function parse_extra_search_response(stdout)
    return parse_extra_search_items(stdout)
end

local function build_extra_search_items(query, class)
    local cmd = make_extra_search_request_args(query, class)
    ...
    if not res.status or res.status ~= 0 then
        msg.verbose("360 搜索请求失败")
        return {}
    end

    return parse_extra_search_items(res.stdout)
end

function get_extra_search_items(query, class)
    return build_extra_search_items(query, class)
end

local function normalize_auto_title(title)
...
local function split_auto_title_year(title)
    title = tostring(title or "")
    local year = title:match("%((%d%d%d%d)%)") or title:match("%f[%d](%d%d%d%d)%f[%D]")
...
local function get_leading_chinese_title(title)
    local prefix = tostring(title or ""):match("^([^%w%s%p]+)")
    if prefix and prefix ~= "" and is_chinese(prefix) then
        return prefix
    end
    return nil
end

local function query_tmdb_auto_tv_title(title, year)
    if not year or options.tmdb_api_key == "" or #Base64.decode(options.tmdb_api_key) < 32 then
        return nil
    end

    local encoded_title = url_encode(title)
    local url = string.format(
        "https://api.themoviedb.org/3/search/tv?api_key=%s&query=%s&language=zh-CN&first_air_date_year=%s",
        Base64.decode(options.tmdb_api_key), encoded_title, url_encode(year)
    )
    local cmd = {
        "curl",
        "-s",
    ...
    if not res.status or res.status ~= 0 then
        msg.info("自动搜索关联跳过：TMDB 查询失败或超时")
        return nil
    end

    local data = utils.parse_json(res.stdout)
    if not data or not data.results then return nil end

    local candidates = {}
    for _, item in ipairs(data.results) do
        local item_year = tostring(item.first_air_date or ""):match("^(%d%d%d%d)")
        local name = item.name
        if item_year == tostring(year) and name and name ~= "" and is_chinese(name) then
    ...
    if #candidates > 1 then
        msg.info("自动搜索关联跳过：TMDB 同年份候选不唯一")
    end
    return nil
end

local function build_auto_search_titles(title)
    local clean_title, year = split_auto_title_year(title)
    local titles = {}
    local seen = {}
    ...
    if is_chinese(clean_title) then
        local chinese_title = get_leading_chinese_title(clean_title)
        if chinese_title and chinese_title ~= clean_title then
            add(chinese_title)
        end
        add(clean_title)
    elseif options.auto_search_tmdb then
        local translated = query_tmdb_auto_tv_title(clean_title, year)
        if translated then add(translated) end
        add(clean_title)
    else
        msg.info("自动搜索关联跳过英文片名 TMDB 查询：auto_search_tmdb 未开启")
    end

    return titles
end

local function get_item_auto_meta(item)
...
local function choose_strict_extra_candidate(items, title)
    local wanted = normalize_auto_title(title)
    if wanted == "" then return nil, "empty title" end

    local works = {}
    local order = {}
    for _, item in ipairs(items or {}) do
        local meta = get_item_auto_meta(item)
        if meta and (meta.class == "电视剧" or meta.class == "动漫") and normalize_auto_title(meta.title) == wanted then
            local key = table.concat({ tostring(meta.class), tostring(meta.id), normalize_auto_title(meta.title), tostring(meta.year or "") }, "|")
            if not works[key] then
                works[key] = { meta = meta, sources = {} }
                table.insert(order, key)
            end
            table.insert(works[key].sources, meta)
        end
    end

    if #order == 0 then
        return nil, "no exact title"
    end
    if #order > 1 then
        return nil, "multiple exact works"
    end

    local work = works[order[1]]
    for _, preferred in ipairs(SourceOrder) do
        for _, meta in ipairs(work.sources) do
            if meta.site == preferred.id then
                return meta
            end
        end
    end
    return work.sources[1]
end

function auto_associate_extra_danmaku()
    if not options.auto_search_associate then return false end

    local title, season_num, episode_num = parse_title()
    episode_num = tonumber(episode_num)
    if not title or title == "" or not episode_num then
        msg.info("自动搜索关联跳过：无法解析明确片名和集数")
        return false
    end

    local last_reason = nil
    for _, search_title in ipairs(build_auto_search_titles(title)) do
        local items = build_extra_search_items(search_title, nil)
        local candidate, reason = choose_strict_extra_candidate(items, search_title)
        if candidate then
            msg.info(string.format(
                "自动搜索关联命中：%s 第%s集 [%s/%s]",
                tostring(candidate.title), tostring(episode_num), tostring(candidate.class), tostring(candidate.site)
            ))
            get_details(candidate.class, candidate.id, candidate.site, candidate.title, candidate.year, nil, episode_num)
            return true
        end
        last_reason = reason
    end

    msg.info("自动搜索关联跳过：" .. tostring(last_reason or "no candidate"))
    return false
end

local function search_query(query, class, menu)
    local items = build_extra_search_items(query, class)
    if #items > 0 then
        if uosc_available then
